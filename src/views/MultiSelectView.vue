...
<template>
    <div class="flex flex-row w-1/2">
        <div class="flex flex-row grow bg-white rounded-md border-gray-200 border-2 hover:border-blue-600 transition-all">
            <div class="flex flex-row w-full flex-wrap px-4 align-middle">
                <div v-for="filter in filters" :key="filter" class="bg-gray-200 px-4 py-2 rounded-md items-center m-1">
                    <span class="pr-2">{{ filter }}</span>
                    <button class="border-l-2 border-gray-400 text-gray-400 pl-2" @click="removeFilter(filter)">X</button>
                </div>
                <input v-model="inputValue" @keydown.enter="addFilter()" class="flex outline-none py-2 m-1" placeholder="Type to filter..."/>
            </div>
            <!-- Search Icon -->
            <div class="flex items-center px-2">
                <svg xmlns="http://www.w3.org/2000/svg" height="40" viewBox="0 -960 960 960" width="40" fill="#e5e7eb">
                    <path
